...
const DFNSItem={
 ...
 async init(){
  ...
  try{
   ...
   const j=await r.json(); if(!j?.data)throw new Error("The API returned no cosmetic data.");
   this.item=j.data; await this.enrichMetadata(id); await this.loadShop(id); await this.loadHistoricalPrice(id); this.buildHistory(); this.render();
 ...
 async enrichMetadata(id){
  const urls=[`${API}/cosmetics/br/${encodeURIComponent(id)}?language=en&responseFlags=${SHOP_HISTORY_FLAG}`,`${API}/cosmetics/br/search/ids?id=${encodeURIComponent(id)}&language=en&responseFlags=${SHOP_HISTORY_FLAG}`,`${API}/cosmetics/br/search/all?id=${encodeURIComponent(id)}&matchMethod=full&language=en&searchLanguage=en&responseFlags=${SHOP_HISTORY_FLAG}`];
  for(const url of urls){try{const r=await fetch(url,{cache:"no-store"});if(!r.ok)continue;const j=await r.json();const list=Array.isArray(j?.data)?j.data:(j?.data?[j.data]:[]);const x=list.find(v=>String(v?.id||"").toLowerCase()===String(id).toLowerCase())||list[0];if(x)this.item={...this.item,...x};if(this.item?.lastAppearance||(Array.isArray(this.item?.shopHistory)&&this.item.shopHistory.length))break}catch(e){console.warn("DFNS metadata:",e)}}
 },
 async loadShop(id){try{const r=await fetch(`${API}/shop`,{cache:"no-store"});if(!r.ok)return;const j=await r.json(),entries=Array.isArray(j?.data?.entries)?j.data.entries:[];this.shopEntry=entries.find(e=>(e.brItems||[]).some(i=>String(i?.id||"").toLowerCase()===String(id).toLowerCase()))||null}catch(e){console.warn("DFNS shop:",e)}},
 async loadHistoricalPrice(id){
  this.historicalPrice=null;
  if(this.shopEntry){const current=this.priceFromShopEntry(this.shopEntry,id);if(current!=null){this.historicalPrice=current;return}}
  const dates=[],history=Array.isArray(this.item?.shopHistory)?this.item.shopHistory:[],last=this.toDate(this.item?.lastAppearance);if(last)dates.push(this.isoDay(last));
  for(const value of history.map(x=>this.toDate(x)).filter(Boolean).sort((a,b)=>b-a)){const d=this.isoDay(value);if(!dates.includes(d))dates.push(d)}
  for(const date of dates.slice(0,12)){try{const r=await fetch(`${HISTORY_RAW}/${date}.json`,{cache:"no-store"});if(!r.ok)continue;const j=await r.json(),entries=Array.isArray(j?.data?.entries)?j.data.entries:Array.isArray(j?.entries)?j.entries:[],hit=entries.find(e=>(e.brItems||[]).some(i=>String(i?.id||"").toLowerCase()===String(id).toLowerCase()));if(hit){const p=this.priceFromShopEntry(hit,id);if(p!=null){this.historicalPrice=p;return}}}catch(e){console.warn("DFNS historical price:",e)}}
  const fallback=this.number(this.item?.price??this.item?.finalPrice??this.item?.regularPrice);if(fallback!=null)this.historicalPrice=fallback;
 },
 priceFromShopEntry(entry,id){for(const v of [entry?.finalPrice,entry?.regularPrice,entry?.price,entry?.prices?.finalPrice,entry?.prices?.regularPrice]){const n=this.number(v);if(n!=null)return n}const item=(entry?.brItems||[]).find(i=>String(i?.id||"").toLowerCase()===String(id).toLowerCase());if(item)for(const v of [item.finalPrice,item.regularPrice,item.price,item.prices?.finalPrice,item.prices?.regularPrice]){const n=this.number(v);if(n!=null)return n}return null},
 buildHistory(){const raw=Array.isArray(this.item?.shopHistory)?this.item.shopHistory:[],appearances=raw.map(x=>this.toDate(x)).filter(Boolean).sort((a,b)=>b-a),directLast=this.toDate(this.item?.lastAppearance),last=directLast||appearances[0]||null,first=this.toDate(this.item?.added)||appearances.at(-1)||null;this.historyRecord={firstSeen:first,lastSeen:last,appearances,price:this.historicalPrice}},
 getLastSeen(){return this.historyRecord?.lastSeen||null},
 getPrice(){if(this.shopEntry){const p=this.priceFromShopEntry(this.shopEntry,this.item?.id);if(p!=null)return p}if(this.historicalPrice!=null)return this.historicalPrice;return this.number(this.item?.price??this.item?.finalPrice??this.item?.regularPrice)},
 render(){const i=this.item,n=i.name||"Unknown Item",type=i.type?.displayValue||i.type?.value||i.displayType||"Cosmetic",rarity=i.rarity?.displayValue||i.rarity?.value||i.displayRarity||"Unknown",set=i.set?.text||i.set?.name||"—",series=i.series?.name||i.series?.value||"—",intro=i.introduction?.text||i.introduction?.chapter||i.introduction?.season||"—",added=this.historyRecord?.firstSeen,last=this.getLastSeen(),price=this.getPrice(),available=!!this.shopEntry;this.text("#item-name",n);this.text("#breadcrumb-item-name",n);this.text("#item-description",i.description||i.shortDescription||"No description available for this cosmetic.");this.text("#item-type",type);this.text("#item-rarity",rarity);this.text("#detail-name",n);this.text("#detail-type",type);this.text("#detail-rarity",rarity);this.text("#detail-id",i.id||"—");this.text("#detail-introduction",intro);this.text("#detail-added",this.formatDate(added));this.text("#detail-set",set);this.text("#detail-series",series);this.text("#detail-set-part",i.set?.partOfSet||set);this.text("#detail-shop-status",available?"In today's shop":"Not currently listed");this.text("#item-price",price==null?"—":price.toLocaleString());this.text("#availability-text",available?"Available in today's shop":last?"Previously available in the Item Shop":"Not currently in the shop");this.text("#availability-date",available?"Live shop data":last?`Last seen ${this.formatDate(last)}`:"No historical shop appearance recorded");this.renderLastSeen(last);this.setImage();this.updateFavorite(this.isFavorite(i.id));this.syncNotifyButton();document.title=`${n} — DFNS`;document.body.classList.add("item-ready")},
 ...
 getFavorites(){try{const x=JSON.parse(localStorage.getItem(this.favoriteKey)||"[]");return new Set(Array.isArray(x)?x:[])}catch{return new Set()}},isFavorite(id){return this.getFavorites().has(id)},
 ...
 text(s,v){const e=document.querySelector(s);if(e)e.textContent=v??"—"},showError(m){document.querySelectorAll("[data-item-error]").forEach(e=>{e.hidden=false;e.textContent=m});console.error("DFNS item error:",m)}
};
