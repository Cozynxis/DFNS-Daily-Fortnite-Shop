...
const DFNSItem = {
  ...
  renderLastSeen(date) { const container = document.querySelector("#detail-last-seen"); if (!container) return; if (!date) { container.textContent = "Never seen / unavailable"; container.removeAttribute("title"); return; } const dateText = this.formatFullDate(date), label = this.lastSeenLabel(date); container.textContent = label; container.title = `Last seen: ${dateText}`; container.classList.add("last-seen-value"); },
  renderRelated() {
  ...
  toDate(value) { if (value === undefined || value === null || value === "") return null; const date = typeof value === "number" ? new Date(value < 10000000000 ? value * 1000 : value) : new Date(value); return Number.isNaN(date.getTime()) ? null : date; },
  ...
  lastSeenLabel(date) { if (!date) return "Never seen / unavailable"; const full = this.formatFullDate(date), now = new Date(), today = new Date(now.getFullYear(), now.getMonth(), now.getDate()), target = new Date(date.getFullYear(), date.getMonth(), date.getDate()), days = Math.max(0, Math.floor((today-target)/86400000)); return `${full} · ${days === 0 ? "today" : days === 1 ? "1 day ago" : `${days.toLocaleString()} days ago`}`; },
