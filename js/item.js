...
const DATAMINING_HISTORY="https://raw.githubusercontent.com/Fortnite-Datamining/Fortnite-Datamining/history/shop/";
const KNOWN_HISTORY={eid_fresh:{firstSeen:"2017-12-16",lastSeen:null,price:800,appearances:[]}};
const DFNSItem={
 item:null,shopEntry:null,imageMode:"featured",favoriteKey:"dfns-favorites",historyRecord:null,
 async init(){
  this.bind();
 ...
 async enrichMetadata(id){
  const urls=[`${API}/cosmetics/br/search/ids?id=${encodeURIComponent(id)}&language=en`,`${API}/cosmetics/br/search/all?id=${encodeURIComponent(id)}&matchMethod=full&language=en&searchLanguage=en`,`${API}/cosmetics/br/search?id=${encodeURIComponent(id)}&matchMethod=full&language=en&searchLanguage=en`];
  for(const url of urls){try{const r=await fetch(url,{cache:"no-store"});if(!r.ok)continue;const j=await r.json();const list=Array.isArray(j?.data)?j.data:(j?.data?[j.data]:[]);const x=list.find(v=>String(v?.id||"").toLowerCase()===String(id).toLowerCase())||list[0];if(x){this.item={...this.item,...x};if(Array.isArray(x.shopHistory)&&x.shopHistory.length)break}}catch(e){console.warn("DFNS metadata:",e)}}
 },
 async loadShop(id){
 ...
 buildHistory(){
  const id=String(this.item?.id||"").toLowerCase(),known=KNOWN_HISTORY[id]||{};
  const raw=Array.isArray(this.item?.shopHistory)?this.item.shopHistory:[];
  const appearances=raw.map(x=>({date:this.toDate(x?.date??x),price:this.number(x?.price??x?.finalPrice??x?.regularPrice)})).filter(x=>x.date).sort((a,b)=>b.date-a.date);
  const directLast=this.toDate(this.item?.lastAppearance);
  const historyLast=appearances[0]?.date||null;
  /* shopHistory is the authoritative lifetime list. lastAppearance is only a fallback. */
  this.historyRecord={firstSeen:this.toDate(this.item?.added)||this.toDate(this.item?.addedSince)||this.toDate(known.firstSeen),lastSeen:historyLast||directLast||this.toDate(known.lastSeen)||null,price:this.number(known.price),appearances}
 },
 async loadHistoricalPrice(){
  if(this.shopEntry)return;
  const entries=this.historyRecord?.appearances||[];
  const last=entries[0]?.date||this.toDate(this.item?.lastAppearance);
  if(!last)return;
  const date=this.utcDateKey(last);
  if(!date)return;
  try{
   const r=await fetch(`${DATAMINING_HISTORY}${date}.json`,{cache:"no-store"});
   if(!r.ok)return;
   const j=await r.json();
   const shopEntries=Array.isArray(j?.entries)?j.entries:(Array.isArray(j?.data?.entries)?j.data.entries:[]);
   const entry=shopEntries.find(e=>(e?.brItems||e?.items||[]).some(i=>String(i?.id).toLowerCase()===String(this.item.id).toLowerCase()));
   if(!entry)return;
   const price=this.number(entry.finalPrice??entry.regularPrice??entry.prices?.[0]?.finalPrice??entry.prices?.[0]?.regularPrice);
   if(price!=null)this.historyRecord.price=price;
   this.historyRecord.historicalShopEntry=entry;
  }catch(e){console.warn("DFNS historical shop price:",e)}
 },
 getLastSeen(){return this.historyRecord?.lastSeen||null},
 getPrice(){
  if(this.shopEntry){const p=this.number(this.shopEntry.finalPrice??this.shopEntry.regularPrice??this.shopEntry.prices?.[0]?.finalPrice??this.shopEntry.prices?.[0]?.regularPrice);if(p!=null)return p}
  const historical=this.historyRecord?.historicalShopEntry;
  if(historical){const p=this.number(historical.finalPrice??historical.regularPrice??historical.prices?.[0]?.finalPrice??historical.prices?.[0]?.regularPrice);if(p!=null)return p}
  const recorded=this.historyRecord?.appearances?.find(x=>x.price!=null)?.price;
  return recorded??this.number(this.item?.price)??this.historyRecord?.price??null
 },
 render(){
 ...
 utcDateKey(d){if(!d)return null;const x=new Date(d);return `${x.getUTCFullYear()}-${String(x.getUTCMonth()+1).padStart(2,"0")}-${String(x.getUTCDate()).padStart(2,"0")}`},
