...
const DFNSReactions = {
  ...
  async load() {
    if (!this.configured()) {
      this.loadLocal();
      this.renderStatus("Community reactions are ready — backend setup is still pending.");
      return;
    }

    try {
      const filter = encodeURIComponent(this.state.id);
      const response = await fetch(`${this.config.url}/rest/v1/${this.table}?select=vote,voter_id&cosmetic_id=eq.${filter}`, {
      ...
      const mine = rows.find(x => x.voter_id === this.voterId);
      this.state.vote = mine?.vote || null;
      this.render();
      this.renderStatus("Your reaction is saved for this cosmetic.");
    } catch (error) {
      console.warn("DFNS reactions backend unavailable:", error);
      this.loadLocal();
      this.renderStatus("Showing local reactions while the community database reconnects.");
    }
  },
  ...
  loadLocal() {
    try {
      ...
      this.state.fire = 0; this.state.poop = 0; this.state.vote = null;
    }
    this.render();
  },
  ...
  async vote(nextVote) {
    ...
    try {
      if (!this.configured()) {
        ...
        const response = await fetch(`${this.config.url}/rest/v1/${this.table}?on_conflict=cosmetic_id,voter_id`, {
          method: "POST",
        ...
        const response = await fetch(`${this.config.url}/rest/v1/${this.table}?cosmetic_id=eq.${encodeURIComponent(this.state.id)}&voter_id=eq.${encodeURIComponent(this.voterId)}`, {
          method: "DELETE",
  ...
  render() {
    ...
    const labels = document.querySelectorAll("[data-dfns-reaction-percent]");
    labels.forEach(label => label.textContent = label.dataset.dfnsReactionPercent === "fire" ? `${firePct}%` : `${poopPct}%`);
  },
  ...
  flash(text) {
    ...
    this.flashTimer = setTimeout(() => this.renderStatus("Click a reaction to change your vote."), 2200);
  },
