...
const DFNSReactions = {
  ...
  configured() { return Boolean(this.config?.url && this.config?.anonKey); },

  apiBase() { return String(this.config.url || "").replace(/\/+$/, "").replace(/\/rest\/v1$/, ""); },

  headers(extra = {}) {
    return { apikey: this.config.anonKey, Authorization: `Bearer ${this.config.anonKey}`, "Content-Type": "application/json", ...extra };
  },
  ...
  async load() {
    ...
    try {
      ...
      const response = await fetch(`${this.apiBase()}/rest/v1/${this.table}?select=vote,voter_id&cosmetic_id=eq.${filter}`, { headers: this.headers({ Accept: "application/json" }), cache: "no-store" });
  ...
  async vote(nextVote) {
    ...
    try {
      if (!this.configured()) {
        ...
        const response = await fetch(`${this.apiBase()}/rest/v1/${this.table}?on_conflict=cosmetic_id,voter_id`, { method: "POST", headers: this.headers({ Prefer: "resolution=merge-duplicates,return=minimal" }), body: JSON.stringify({ cosmetic_id: this.state.id, voter_id: this.voterId, vote: next, updated_at: new Date().toISOString() }) });
        if (!response.ok) throw new Error(`Vote save failed (${response.status})`);
      } else {
        const response = await fetch(`${this.apiBase()}/rest/v1/${this.table}?cosmetic_id=eq.${encodeURIComponent(this.state.id)}&voter_id=eq.${encodeURIComponent(this.voterId)}`, { method: "DELETE", headers: this.headers({ Prefer: "return=minimal" }) });
  ...
  saveLocal(vote) {
    ...
    if (item.voters[this.voterId] === "poop") item.poop = Math.max(0, item.poop - 1);
  ...
  render() {
    ...
    const fireButton = document.querySelector('[data-dfns-reaction="fire"]'), poopButton = document.querySelector('[data-dfns-reaction="poop"]');
    ...
    if (fireButton) { fireButton.classList.toggle("selected", this.state.vote === "fire"); fireButton.setAttribute("aria-pressed", String(this.state.vote === "fire")); }
    if (poopButton) { poopButton.classList.toggle("selected", this.state.vote === "poop"); poopButton.setAttribute("aria-pressed", String(this.state.vote === "poop")); }
    document.querySelectorAll("[data-dfns-reaction-percent]").forEach(label => label.textContent = label.dataset.dfnsReactionPercent === "fire" ? `${firePct}%` : `${poopPct}%`);
  },

  renderStatus(text) { const el = document.querySelector("#dfns-reaction-status"); if (el) el.textContent = text; },
  flash(text) { const el = document.querySelector("#dfns-reaction-status"); if (!el) return; el.textContent = text; clearTimeout(this.flashTimer); this.flashTimer = setTimeout(() => this.renderStatus("Tap a reaction to change your vote."), 2200); },
  format(value) { return Number(value || 0).toLocaleString("en-US"); }
};
