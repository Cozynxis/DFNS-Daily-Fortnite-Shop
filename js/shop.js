...
function getFavorites() {
  try {
    const value = JSON.parse(localStorage.getItem("dfns-favorites") || "[]");
...
function toggleFavorite(id) {
  ...
  localStorage.setItem("dfns-favorites", JSON.stringify([...favorites]));
  return !active;
}
...
function normalize(data) {
  ...
  for (const entry of data?.entries || []) {
    for (const c of entry?.brItems || []) {
      ...
      out.push({
        id: c.id,
        name: c.name || c.displayName || "Unknown Item",
        type: type(c),
        rarity: rarity(c),
        image: image(c),
        price: Number(entry.finalPrice ?? entry.regularPrice ?? 0),
        section: String(entry.section?.name || entry.section?.displayName || "Daily").toLowerCase()
      });
    }
  }
  return out;
  ...
  return `<article class="shop-card">
    <a class="shop-card-link" href="item.html?id=${encodeURIComponent(item.id)}">
      <div class="shop-card-image-wrapper">
        ${item.image ? `<img class="shop-card-image" src="${esc(item.image)}" alt="${esc(item.name)}" loading="lazy">` : `<div class="shop-card-image-placeholder">No image</div>`}
        <span class="shop-card-rarity">${esc(item.rarity)}</span>
      </div>
      <div class="shop-card-content">
        <h3 class="shop-card-name">${esc(item.name)}</h3>
        <div class="shop-card-meta">
          <span class="shop-card-type">${esc(item.type)}</span>
          <span class="shop-card-price"><span class="vbucks-icon">V</span> ${item.price.toLocaleString()}</span>
        </div>
      </div>
    </a>
    <button class="shop-card-favorite ${favorite ? "active" : ""}" type="button" data-favorite-id="${esc(item.id)}" aria-label="${favorite ? "Remove" : "Add"} favorite">${favorite ? "♥" : "♡"}</button>
  </article>`;
}
...
function render() {
  ...
  const f = document.querySelector("#featured-items");
  const d = document.querySelector("#daily-items");
  ...
  setText("#visible-item-count", items.length.toLocaleString());
  setText("#shop-item-count", items.length.toLocaleString());
  setText("#featured-count", featuredItems.length.toLocaleString());
  setText("#daily-count", daily.length.toLocaleString());
  setText("#active-filter-summary", state.search || state.category !== "all" || state.rarity !== "all" || state.price !== "all" ? "Filters active" : "All Items");
}
...
async function load() {
  try {
    ...
    state.items = normalize(json.data);
    ...
    document.querySelectorAll(".shop-grid").forEach(grid => {
      grid.innerHTML = `<div class="shop-api-error"><strong>Unable to load today's shop.</strong><span>${esc(error.message)}</span><button type="button" onclick="location.reload()">Retry</button></div>`;
    });
  }
}
...
document.addEventListener("click", event => {
  ...
  const active = toggleFavorite(button.dataset.favoriteId);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  state.search = (query.get("search") || "").toLowerCase();
  ...
  document.querySelectorAll("[data-filter-type]").forEach(button => button.addEventListener("click", () => {
    ...
    state[filterType] = button.dataset.filterValue || "all";
    document.querySelectorAll(`[data-filter-type="${filterType}"]`).forEach(x => x.classList.toggle("active", x === button));
    render();
  }));

  document.querySelector("#reset-filters")?.addEventListener("click", () => {
    Object.assign(state, { search: "", category: "all", rarity: "all", price: "all", sort: "default" });
    if (input) input.value = "";
    const sort = document.querySelector("#shop-sort-select");
    if (sort) sort.value = "default";
    document.querySelectorAll("[data-filter-type]").forEach(x => x.classList.toggle("active", x.dataset.filterValue === "all"));
    render();
  });
  ...
  document.querySelectorAll("[data-view]").forEach(button => button.addEventListener("click", () => {
    ...
    document.querySelectorAll(".shop-grid").forEach(grid => grid.classList.toggle("compact-view", view === "compact"));
  }));
